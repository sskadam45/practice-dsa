  ...
  return (
    ...
    <tbody className='text-white'>
        {problems.map((doc,idx)=>{
            const difficultyColor = doc.difficulty === "Easy" ? "text-green-s": doc.difficulty === "Medium" ? "text-dark-yellow": "text-dark-pink";
            return (
                <tr className={`${idx %2 === 1 ? "bg-dark-layer-1": ""}`} key={doc.id}>
                    ...
                    <td className='px-6 py-4'>
                        <Link className="hover:text-blue-500 cursor-pointer" href={`/problems/${doc.id}`}>{doc.title}</Link>
                    </td>
                    ...
                    <td className={'px-6 py-4'}>
                        {doc.videoId ? (<AiFillYoutube fontSize={"30"} className='cursor-pointer hover:text-red-600'
